...
import com.openquartz.sequence.core.uid.snowflake.worker.AbstractIntervalWorkId;
...
public class ZkWorkerIdAssigner extends AbstractIntervalWorkId {
    ...
    public long action() {
        try {
            zkClient = new ZooKeeper(zkAddress, SESSION_TIMEOUT, event -> {
            ...
            log.error("[ZkWorkerIdAssigner#action]error", ex);
            Thread.currentThread().interrupt();
        } catch (KeeperException | IOException e) {
            log.error("[ZkWorkerIdAssigner#action]error", e);
        }
    ...
    public void report() {
        try {
            ...
            log.error("[ZkWorkerIdAssigner#report]report-error", e);
            Thread.currentThread().interrupt();
